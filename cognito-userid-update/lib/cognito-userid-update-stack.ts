...
export class CognitoUSerIdUpdateStack extends cdk.Stack {
  ...
  private createUserPool() {
    const userPool = new cognito.UserPool(this, 'SiteUserPool', {
      ...
      // Store a unique GUID as the users ID
      customAttributes: {
        userId: new cognito.StringAttribute({ minLen: 36, maxLen: 36, mutable: true }),
      },
    });
  ...
  private createApiGateway(userProfileTable: dynamodb.Table, userPool: cognito.UserPool) {
    ...
    const api = new apigateway.RestApi(this, 'UserProfileApi', {
      ...
      description: 'This service serves user profiles for user ID.',
